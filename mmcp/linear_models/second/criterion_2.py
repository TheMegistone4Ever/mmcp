...
def solve(c, A, b, d, z_min, alpha):
    ...
    """
    logging.debug(f"Entering solve function in criterion_2.py (linear_models/second) with: "
    ...
    if solver_status != pywraplp.Solver.OPTIMAL:
        logging.error(f"Unable to find the optimal solution for the second linear model, second criterion. "
                      f"{solver_status=}")
    ...
    logging.info(f"Optimal solution found for the second linear model, second criterion: "
