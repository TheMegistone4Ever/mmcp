...
def solve(c, A, b, weights):
    ...
    """
    logging.debug(f"Entering solve function in criterion_3.py (linear_models/first) with: "
    ...
    for iteration in range(max_iterations):
        ...
        if solver_status != pywraplp.Solver.OPTIMAL:
            logging.error(f"Unable to find the optimal solution for the first linear model, third criterion. "
    ...
    logging.info(f"Optimal solution found for the first linear model, third criterion: "
...
def update_weights(weights, x):
    ...
    Returns:
        Updated weights.
    """
    logging.debug(f"Updating weights in criterion_3.py (linear_models/first) with weights={weights}, x={x}")
